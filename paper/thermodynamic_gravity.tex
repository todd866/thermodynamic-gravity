...
\subsection{Why Interfaces Quantize: Dimensional Collapse and Lattice Selection}

The discreteness of certification---quantized entropy increments, finite resolution---is not imposed by hand. It follows from two generic features of dimensional collapse:

\textbf{1. Projection degeneracy (collisions).} When high-dimensional dynamics project to a low-dimensional interface, distinct configurations become indistinguishable. Below a critical embedding dimension, continuous trajectories in the high-dimensional space necessarily overlap in the projection. The interface cannot represent continuous dynamics faithfully; it is forced to partition configurations into equivalence classes.

\textbf{2. Pre-existing structure (the lattice).} The low-dimensional interface is not a blank canvas. It already has causal structure, metric compatibility conditions, and diffeomorphism constraints. New information must be \emph{compatible} with this existing structure. The structure acts as a lattice (or discretizing codebook) that selects which equivalence classes are admissible.

Together: projection forces collisions, and the lattice resolves collisions by selecting admissible outputs. This is quantization. Discreteness is imposed by the interface, not by the high-dimensional configuration space itself.

\textbf{Application to null screens.} Matter dynamics are high-dimensional---the full stress-energy configuration across a spacetime region. The null screen is a low-dimensional interface where those dynamics get certified as admissible. The existing metric structure is the lattice: it determines which deformations can pass through. The Clausius relation $\delta Q = T\,\delta S$ is the admissibility condition. Certification entropy $S = \eta A$ counts lattice states, and the minimum increment $\Delta S_{\min} \sim \mathcal{O}(1)$ reflects the lattice spacing (developed in Section 6).

This perspective clarifies why $\hbar$ appears at interfaces: it sets the resolution of the pre-existing structure. The Planck area $G\hbar$ is not a property of spacetime geometry but of the certification layer's discretizing capacity.

\textbf{The central claim.} Standard quantum mechanics treats quantization as ontological: the substrate itself is discrete at the Planck scale. We propose instead that the substrate can be arbitrarily high-dimensional and continuous. Quantization describes the \emph{communication channel}---the interface where high-dimensional dynamics project to low-dimensional observables. Discreteness is a property of the channel, not of reality. The Clausius residual $\Delta_\Sigma$ is channel noise: the part of the high-dimensional input that does not survive projection. Classical physics corresponds to a noiseless channel; quantum corrections arise from finite channel resolution.
...
\medskip
\noindent\fbox{\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule}{%
\textbf{Summary.} Standard quantum mechanics treats quantization as ontological---the substrate is discrete. We propose that quantization describes the \emph{communication channel}: the interface where high-dimensional dynamics project to low-dimensional observables. Discreteness is a property of the channel, not of reality. Gravity is classical because it describes the high-dimensional embedding; quantum gravity describes the finite resolution of the certification channel.}}
\medskip
